...
def _set_numbering(
    figure, centers, render_numbering=True, numbers_size=None, numbers_colour="k"
):
...
class MayaviRenderer(Renderer):
    ...
    def __init__(self, figure_id, new_figure):
        try:
            ...
            raise ImportError(
                "mayavi is required for viewing 3D objects "
    ...
    def save_figure(
        self, filename, format="png", size=None, magnification="auto", overwrite=False
    ):
        r"""
        ...
        _export(
            savefig_args, filename, self._extensions_map, format, overwrite=overwrite
        )
    ...
    def projection_matrix(self):
        ...
        p = (
            camera.get_projection_transform_matrix(aspect_ratio, -1, 1)
            .to_array()
            .astype(np.float32)
        )
        return p
...
class MayaviTriMeshViewer3d(MayaviRenderer):
    ...
    def render(
        ...
        if normals is not None:
            MayaviVectorViewer3d(self.figure_id, False, self.points, normals).render(
                colour=normals_colour,
...
class MayaviTexturedTriMeshViewer3d(MayaviRenderer):
    def __init__(
        self, figure_id, new_figure, points, trilist, texture, tcoords_per_point
    ):
        super(MayaviTexturedTriMeshViewer3d, self).__init__(figure_id, new_figure)
        self.points = points
        self.trilist = trilist
    ...
    def _render_mesh(
        self, mesh_type="surface", ambient_light=0.0, specular_light=0.0, alpha=1.0
    ):
...
class MayaviColouredTriMeshViewer3d(MayaviRenderer):
    def __init__(self, figure_id, new_figure, points, trilist, colour_per_point):
        super(MayaviColouredTriMeshViewer3d, self).__init__(figure_id, new_figure)
        self.points = points
        self.trilist = trilist
    ...
    def _render_mesh(
        ...
        pd.points = self.points
        pd.polys = self.trilist
        pd.point_data.scalars = (self.colour_per_point * 255.0).astype(np.uint8)
        mapper = tvtk.PolyDataMapper()
        mapper.set_input_data(pd)
        p = tvtk.Property(
            representation=mesh_type,
...
class MayaviLandmarkViewer3d(MayaviRenderer):
    ...
    def render(
        ...
        numbers_size = _parse_marker_size(numbers_size, self.landmark_group.points)
