...
from .derivatives import d_camera_d_camera_parameters, d_camera_d_shape_parameters
...
class LucasKanade(object):
    ...
    def J_lms(self, camera, warped_uv, shape_pc_uv, camera_update, focal_length_update):
    ...
    def _precompute(self):
        ...
        # Priors
        self.J_shape_prior = 1.0 / np.array(self.model.shape_model.eigenvalues)
        self.J_texture_prior = 1.0 / np.array(self.model.texture_model.eigenvalues)
...
class SimultaneousForwardAdditive(LucasKanade):
    ...
    def run(
        ...
        texture_parameters = self.model.project_instance_on_texture_model(initial_mesh)

        # Reconstruct provided instance
        instance = self.model.instance(
            shape_weights=shape_parameters, texture_weights=texture_parameters
        )
        ...
        instance_per_iter = [instance.rescale_texture(0.0, 1.0)]
        costs = None
        if return_costs:
            costs = []
        ...
        while k < max_iters and eps > self.eps:
            ...
            (vertex_indices, bcoords, tri_indices, yx) = self.visible_sample_points(
                instance_in_image, image.shape
            )
            ...
            texture_pc_uv = self.model.sample_texture_model(bcoords, tri_indices)
            ...
            if reconstruction_weight is not None:
                ...
                hessian = np.zeros(
                    (self.n + n_camera_parameters, self.n + n_camera_parameters)
                )
            ...
            if landmarks_prior_weight is not None:
                ...
                warped_lms = instance_in_image.points[self.model.model_landmarks_index]
                ...
                idx = self.n + n_camera_parameters
                hessian[:idx, :idx] += landmarks_prior_weight * sd_lms.dot(sd_lms.T)
            ...
            instance_per_iter.append(instance.rescale_texture(0.0, 1.0))
        ...
        return MMAlgorithmResult(
            ...
            meshes=instance_per_iter,
            camera_transforms=camera_per_iter,
            image=image,
            initial_mesh=initial_mesh.rescale_texture(0.0, 1.0),
            initial_camera_transform=camera_per_iter[0],
            gt_mesh=gt_mesh,
            costs=costs,
        )
...
class WibergForwardAdditive(LucasKanade):
    ...
    def run(
        ...
        while k < max_iters and eps > self.eps:
            ...
            m_texture_uv = (
                self.model.instance().sample_texture_with_barycentric_coordinates(
                    bcoords, tri_indices
                )
            )
            shape_pc_uv = self.sample(self.shape_pc, bcoords, vertex_indices)
            ...
            # Solve to find the increment of parameters
            d_shape, d_camera = self.solve(
                hessian, sd_error, camera_update, focal_length_update
            )
